...
path_svg_files_after_08 = path_svg_files[9:]
#print(path_svg_files_after_08)
is_first_loop=False
#print(path_svg_files[-1])

try:
    plot_svg(path_svg_files[0], ad)
    time.sleep(3)
    plot_svg(path_svg_files[-1], ad)
        # Plot each SVG file in sequence
    for i, path_svg_file in enumerate(path_svg_files_after_08):
        ser.write(b"roll\n")
        while True:
            if ser.in_waiting > 0:
                ...
                if response == "complete":
                    ...
                    print(f"Start AxiDraw of {path_svg_file} ({i+1}/{len_path_svg_files+8})")
                    plot_svg(path_svg_file, ad)
                    break

    while True:
        # Plot each SVG file in sequence
        for i, path_svg_file in enumerate(path_svg_files[:-1]):
